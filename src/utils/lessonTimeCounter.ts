    ...
    Database.getShortenedLessonsStatus().then((result) => {
      ...
      if (now.day() != 6 && now.day() != 0) {
        if (
          ...
          resolve(countDuration("Lekcja", 1, now, lesson1Start, lesson1End, isShortLessons));
        } else if (now.isBetween(lesson1End, lesson2Start)) {
          resolve(countDuration("Przerwa", 1, now, lesson1End, lesson2Start, isShortLessons));
        } else if (
          now.isBetween(lesson2Start, lesson2End) ||
          now.isSame(lesson2Start) ||
          now.isSame(lesson2End)
        ) {
          resolve(countDuration("Lekcja", 2, now, lesson2Start, lesson2End, isShortLessons));
        } else if (now.isBetween(lesson2End, lesson3Start)) {
          resolve(countDuration("Przerwa", 2, now, lesson2End, lesson3Start, isShortLessons));
        } else if (
          now.isBetween(lesson3Start, lesson3End) ||
          now.isSame(lesson3Start) ||
          now.isSame(lesson3End)
        ) {
          resolve(countDuration("Lekcja", 3, now, lesson3Start, lesson3End, isShortLessons));
        } else if (now.isBetween(lesson3End, lesson4Start)) {
          resolve(countDuration("Przerwa", 3, now, lesson3End, lesson4Start, isShortLessons));
        } else if (
          now.isBetween(lesson4Start, lesson4End) ||
          now.isSame(lesson4Start) ||
          now.isSame(lesson4End)
        ) {
          resolve(countDuration("Lekcja", 4, now, lesson4Start, lesson4End, isShortLessons));
        } else if (now.isBetween(lesson4End, lesson5Start)) {
          resolve(countDuration("Przerwa", 4, now, lesson4End, lesson5Start, isShortLessons));
        } else if (
          now.isBetween(lesson5Start, lesson5End) ||
          now.isSame(lesson5Start) ||
          now.isSame(lesson5End)
        ) {
          resolve(countDuration("Lekcja", 5, now, lesson5Start, lesson5End, isShortLessons));
        } else if (now.isBetween(lesson5End, lesson6Start)) {
          resolve(countDuration("Przerwa", 5, now, lesson5End, lesson6Start, isShortLessons));
        } else if (
          now.isBetween(lesson6Start, lesson6End) ||
          now.isSame(lesson6Start) ||
          now.isSame(lesson6End)
        ) {
          resolve(countDuration("Lekcja", 6, now, lesson6Start, lesson6End, isShortLessons));
        } else if (now.isBetween(lesson6End, lesson7Start)) {
          resolve(countDuration("Przerwa", 6, now, lesson6End, lesson7Start, isShortLessons));
        } else if (
          now.isBetween(lesson7Start, lesson7End) ||
          now.isSame(lesson7Start) ||
          now.isSame(lesson7End)
        ) {
          resolve(countDuration("Lekcja", 7, now, lesson7Start, lesson7End, isShortLessons));
        } else if (now.isBetween(lesson7End, lesson8Start)) {
          resolve(countDuration("Przerwa", 7, now, lesson7End, lesson8Start, isShortLessons));
        } else if (
          now.isBetween(lesson8Start, lesson8End) ||
          now.isSame(lesson8Start) ||
          now.isSame(lesson8End)
        ) {
          resolve(countDuration("Lekcja", 8, now, lesson8Start, lesson8End, isShortLessons));
        } else if (now.isAfter(lesson8End) && now.isBefore(night)) {
        /* else if (now.isBetween(lesson8End, lesson9Start)) {
                countDuration("Przerwa", 8, now, lesson8End, lesson9Start, isShortLessons)
            } else if (now.isBetween(lesson9Start, lesson9End) || now.isSame(lesson9Start) || now.isSame(lesson9End)) {
                countDuration("Lekcja", 9, now, lesson9Start, lesson9End, isShortLessons)
            } */
          ...
          resolve(countDuration("morning", 0, now, lesson1Start, lesson1Start, isShortLessons));
        }
      } else {
...
function countDuration(type: "Lekcja" | "Przerwa" | "morning",  number: number, now: moment.Moment, timeStart: moment.Moment, timeEnd: moment.Moment, isShortLessons: boolean): Object {
    ...
    duration = duration === "godzina" ? "45 minut" : duration;
